...
namespace UnityEngine {
    public struct Mathd {
        ...
        public const double ComparationEpsilon = 1E-2d;
        ...
        public static bool RoughlyEqual(double a, double b) {
            return System.Math.Abs(a - b) < Mathd.ComparationEpsilon;
        }

        public static bool RoughlyEqual(double a, double b, double epsilon) {
            return System.Math.Abs(a - b) < epsilon;
        }

        public static bool AnglesRoughlyEqual(double angleDegrees1, double angleDegrees2, double epsilon) {
            double a1 = NormalizeAngleInDegrees(angleDegrees1);
            double a2 = NormalizeAngleInDegrees(angleDegrees2);
            return Math.Abs(a1 - a2) < epsilon;
        }


        public enum InterpolationCurve { LINEAR, EASE_IN_OUT_CUBIC, EASE_OUT_CUBIC, PARABOLIC};

        public static double GetInterpolationRatio(double min, double max, double v, InterpolationCurve curve) {
            switch (curve) {
                case InterpolationCurve.EASE_IN_OUT_CUBIC:
                    return EaseInOutCubicRatio(min, max, v);
                case InterpolationCurve.EASE_OUT_CUBIC:
                    return EaseOutCubicRatio(min, max, v);
                case InterpolationCurve.PARABOLIC:
                    return ParabolicRatio(min, max, v);
                default:
                    return InverseLerp(min, max, v);
            }
        }

        //public static double LinearRatio(double min, double max, double v) {
        //    return Clamp((v - min) / (max - min), 0, 1);
        //}

        public static double EaseInOutCubicRatio(double min, double max, double v) {
            double x = InverseLerp(min, max, v);
            return x < 0.5 ? 4 * x * x * x : 1 - Mathd.Pow(-2 * x + 2, 3) / 2;
        }

        public static double EaseOutCubicRatio(double min, double max, double v) {
            double x = InverseLerp(min, max, v);
            return 1 - Mathd.Pow(1 - x, 3);
        }

        public static double ParabolicRatio(double min, double max, double v)
        {
            double x = InverseLerp(min, max, v);
            return 1 - (Pow((2 * (x - 0.5)), 2));
        }

        public static double NormalizeAngleInDegrees(double a) {
            while (a < 0) a += 360;
            if (Math.Abs(a - 360) < 0.01) return 0;
            a = a % 360;
            return a >= 0 ? a : 360 + a;
        }

        public static bool RangesOverlap(double min1, double max1, double min2, double max2, double epsilon=0)
        {
            return !((min2 >= max1 + epsilon) || (max2 <= min1 - epsilon));
        }
